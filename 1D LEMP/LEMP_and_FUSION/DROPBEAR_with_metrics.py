...
Metric = make_dataclass("Metric", [("method", str), ("nodes", int),("filter", bool), ("MSE", float),("SNR", float),("TRAC", float)])
trial =[]
...
modes=4
meshes = [101,111,121]
samp_method = ['Bayesian','Metropolis-Hastings','liklihood ratio test','Gibbs','no']
for nodes in meshes:
    for kalman in ['filtered','unfiltered']:
        for mi,method in enumerate(samp_method):
            trac_t = []
            mse_t  = []
            snr_t  = []
            for t in range(trails):
                print(str(int((t/trails*100)+1))+'%')
                # print(method)
                #%% Load Data
                ...
                # K[4,4]=data["pin_equivalent_stiffness"]
                ...
                trac_t.append(mt.TRAC(measured_pin_location, estimated_pin_location_len)) 
                mse_t.append(mt.MSE(measured_pin_location, estimated_pin_location_len)) 
                snr_t.append(mt.SNR(measured_pin_location, estimated_pin_location_len)) 
                # trac = mt.TRAC(measured_pin_location, estimated_pin_location_len)
                # mse  = mt.MSE( measured_pin_location, estimated_pin_location_len)
                # snr  = mt.SNR( measured_pin_location, estimated_pin_location_len)
        
                tag='{} node model {} sampled with {} algorithm'.format(nodes,kalman,method)
            trac = np.average(trac_t)
            mse  = np.average(mse_t)
            snr  = np.average(snr_t)
            print(tag)
            trial.append(Metric(method,nodes,kalman=='filtered',mse,snr,trac))
dataset= pd.DataFrame(trial)
m=''
for mesh in meshes:
    m+=str(mesh)+','
m=m[0:-1]
dataset.to_csv('meshes_{}_metrics_average.csv'.format(m))        
